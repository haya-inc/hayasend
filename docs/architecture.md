...
The current bootstrap bearer key is passed to Lambda as an encrypted
environment variable by CloudFormation. Scoped, hashed API-key records and
rotation are required before v1.
...
3. SQS accepts a `send_email` job.
4. The worker loads current state, skips final or canceled messages, and calls
   SES v2.
