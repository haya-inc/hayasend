...
export const PORTABLE_RUNTIME_CAPABILITIES =
  runtimeCapabilityDocumentSchema.parse({
    ...
    required_plan:
      "Customer-controlled containers, PostgreSQL 18, and either private S3-compatible, Google Cloud Storage, or Azure Blob object storage. Platform-native workload identity and secret injection are recommended.",
    limits: {
    ...
    components: {
      atomic_store: {
        status: "supported",
        notes:
          "PostgreSQL transactions atomically persist the message, recipient ledger, idempotency claim, and deterministic outbox intent.",
      },
      recipient_ledger: {
        status: "supported",
        notes:
          "PostgreSQL is the provider-neutral authority for messages, recipients, attempts, immutable events, and outbox state.",
      },
      payload_store: {
        status: "supported",
        notes:
          "One attachment port supports private S3-compatible, Google Cloud Storage, and Azure Blob stores with signed direct uploads, declared checksum metadata, size verification, and final byte-level SHA-256 validation.",
      },
      queue_wakeup: {
        status: "supported",
        notes:
      ...
      periodic_reconciliation: {
        status: "supported",
        notes:
          "Every worker periodically reconciles committed outbox rows and due jobs, recovering after process or wake-up loss.",
      },
      secret_injection: {
        status: "supported",
        notes:
      ...
      webhook_egress: {
        status: "supported",
        notes:
          "Durable PostgreSQL jobs deliver signed webhooks through public-endpoint validation and bounded privacy-safe retries.",
      },
      backup_restore: {
        status: "unsupported",
        notes:
          "PostgreSQL plus object-store backup and isolated restore evidence is still required for every deployment pack.",
      },
    },
    lifecycle: {
      safe_deploy: {
        status: "unsupported",
        notes:
          "Pinned Cloud Run, Azure Container Apps, Render, Railway, and Fly.io deployment packs are not yet published.",
      },
      doctor: {
        status: "conditional",
        notes:
          "Readiness verifies applied PostgreSQL migrations and the selected object-storage container, but platform identity, retention, encryption, and lifecycle policy checks remain open.",
      },
      upgrade: {
        status: "conditional",
        notes:
          "Forward-only additive migrations are checksum-pinned and advisory-lock serialized; exact platform rollout evidence remains open.",
      },
      rollback: {
        status: "conditional",
        notes:
          "Application rollback can retain compatible schema and object state, but each platform pack requires an isolated rollback drill.",
      },
      cleanup: {
        status: "unsupported",
        notes:
          "No portable platform pack has yet published zero-residue cleanup evidence.",
      },
    },
    privacy: {
    ...
    sources: [
      ...
      {
        title: "Google Cloud Storage V4 signed URLs",
        url: "https://cloud.google.com/storage/docs/access-control/signing-urls-with-helpers",
        checked_at: "2026-07-28",
      },
      {
        title: "Azure Blob user delegation SAS",
        url: "https://learn.microsoft.com/en-us/azure/storage/blobs/storage-blob-user-delegation-sas-create-javascript",
        checked_at: "2026-07-28",
      },
      {
