...
if [[ "${HAYASEND_ALLOW_ROLLBACK:-}" != "railway" ]]; then
  echo "Set HAYASEND_ALLOW_ROLLBACK=railway to confirm the reviewed digest rollback." >&2
  exit 1
fi
...
export HAYASEND_IMAGE="$HAYASEND_ROLLBACK_IMAGE"
